          ...
          <div class="col-12">
            <div class="card ">
              <div class="card-header" style="text-align: center;width: auto;">
                <h4> بيانات غيابات الرواتب للشهر المالي  <span style="color:#6777ef "> {{  $finance_cln_periods_data['Month']->name}} </span> لسنة  {{ $finance_cln_periods_data['FINANCE_YR'] }} 
               </h4>
              </div>
                  ...
                  <tbody>
                     ...
                     <td> 
                      ...
                      <div class="dropdown-menu text-center">
                        ...
                        class="dropdown-item has-icon load_edit_this_row " >
                        تعديل</button>

                      <button class="btn btn-danger sm delete_this_row" 
                      data-id="{{ $info->id }}"
                      data-main_salary_employee_id="{{ $info->main_salary_employee_id }}"
                      class="dropdown-item has-icon delete_this_row" >   
                        حذف</button>
                        @endif
                      </div>         
                      </td>
   ...
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-primary">
       ...
       <div class="modal-body bg-white" id="AddModalBody" style="direction: rtl">
         ...
         <div class="col-md-12 text-center">
            <div class="form-group">
               <button id="do_add_now" type="submit" name="submit"class="btn btn-primary" >اضف الغياب</button>
            </div>
         </div>
...
<div class="modal fade" id="EditModal" style="text-align: center" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-primary">
       <div class="modal-header">
         <h4 class="modal-title" style="color: white;text-align:center;width: 100%;">تعديل غياب مسبق</h4>
       </div>
...
<script>
   $(document).ready(function(){
      ...
      $(document).on('click','#do_add_now',function(e){
         ...
         if(total_Add==""){
            alert("من فضلك اجمالي عدد الغياب");
            $("#total_Add").focus();
            return false;
         }
...
$("#backup_freeze_modal").modal("show");
   jQuery.ajax({
   url:'{{ route('Main_salary_employee_absence.checkExsistsBefor') }}',
   type:'post',
   'dataType':'json',
   cache:false,
...
if(flagRes==true){
   ...
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code:employees_code_Add,finance_cln_periods_id:the_finance_cln_periods_id,value:value_Add,total:total_Add,notes:notes_Add,emp_day_price:day_price_add,notes:notes_Add},
   success: function(data){
   ajax_search();
   setTimeout(function() {
   ...
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code:employees_code,is_archived:is_archived,the_finance_cln_periods_id:the_finance_cln_periods_id},
   success: function(data){
...
$("#backup_freeze_modal").modal("show");
   ...
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code:employees_code_edit,the_finance_cln_periods_id:the_finance_cln_periods_id,main_salary_employee_id:main_salary_employee_id,id:id,value:value_edit,total:total_edit,notes:notes_edit,emp_day_price:day_price_edit,notes:notes_edit},
   success: function(data){
   ajax_search();
   setTimeout(function() {
         $("#backup_freeze_modal").modal("hide");
