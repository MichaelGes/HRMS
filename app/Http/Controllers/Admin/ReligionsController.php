...
class ReligionsController extends Controller
{
    public function index()
    {
        ...
        return view('admin.Religion.index',['data'=>$data]);
    }
    ...
    public function store(ReligionsRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('Religion.index')->with(['success' => 'تم ادخال البيانات بنجاح']);
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        if (empty($data)) {
            return redirect()->route('Religion.edit')->with(['error' => 'عفوا غير قادر للوصول الي البيانات المطلوبة !']);
        }
        return view("admin.Religion.edit", ['data' => $data]);
    }
    public function update($id, ReligionsRequest $request)
    {
        try {
            ...
            $data = get_cols_where_row(new Religion(), array("*"), array("com_code" => $com_code, "id" => $id));
            if (empty($data)) {
                return redirect()->route('Religion.index')->with(['error' => 'عفوا غير قادر للوصول الي البيانات المطلوبة !']);
            }
            ...
            DB::commit();
            return redirect()->route('Religion.index')->with(['success' => 'تم تحديث البيانات بنجاح']);
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with(['error' => 'عفوا حدث خطأ ' . $ex->getMessage()])->withInput();
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('Religion.index')->with(['success' => 'تم الحذف البيانات بنجاح']);
        } catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with(['error' => 'عفوا حدث خطأ ' . $ex->getMessage()])->withInput();
        }
