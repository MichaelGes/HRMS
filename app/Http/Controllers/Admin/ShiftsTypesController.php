...
class ShiftsTypesController extends Controller
{
    ...
    public function store(ShiftsTypesRequest $request)
    {
        try {
            ...
            if (!empty($checkExsitsData)) {
                return redirect()->back()->with(['error' => 'عفوا هذة البيانات مسجلة من قبل'])->withInput();
            }
            ...
            DB::commit();
            return redirect()->route('SheftsTypes.index')->with(['success' => 'تم تسجيل البيانات بنجاح']);
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        if (empty($data)) {
            return redirect()->route('SheftsTypes.index')->with(['error' => 'عفوا غير قادر الوصول الي البيانات برجاء الرجوع الي المسؤل']);
        }
        return view('admin.ShiftsTypes.edit', ['data' => $data]);
        
    }
    public function update($id, ShiftsTypesRequest $request){
        try{
            $com_code = auth()->user()->com_code;
            $data = get_cols_where_row(new Shifts_type(), array("*"), array("com_code" => $com_code, 'id' => $id));
            if (empty($data)) {
            ...
            ->where('total_hour','=',$request->total_hour)->where('id','!=',$id)
            ->first();
            if (!empty($checkExsitsData)) {
                return redirect()->back()->with(['error' => 'عفوا هذة البيانات مسجلة من قبل'])->withInput();
            }
            ...
            DB::commit();
            return redirect()->route('SheftsTypes.index')->with(['success'=>'تم تحديث البانات بنجاح']);
        }
        catch (\Exception $ex) {
            DB::rollBack();
            return redirect()->back()->with(['error' => 'عفوا حدث خطا ما ' . $ex->getMessage()])->withInput();
    ...
    public function destroy($id){
        try{
            ...
            DB::commit();
            return redirect()->route('SheftsTypes.index')->with(['success'=>'تم حذف الشيفت بنجاح']);    
        }
            catch (\Exception $ex) {
                DB::rollBack();
                return redirect()->back()->with(['error' => 'عفوا حدث خطا ما ' . $ex->getMessage()])->withInput();
