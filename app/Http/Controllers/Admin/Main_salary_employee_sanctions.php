...
class Main_salary_employee_sanctions extends Controller
{
    public function index()
    {
        ...
        return view('admin.Main_salary_employee_sanctions.index', ['data' => $data]);
    }
    public function show($finance_cln_periods_id)
    {
        ...
        if (empty($finance_cln_periods_data)) {
            return redirect()->route('MainSalaryRecord.index')->with(['error' => 'عفوا غير قادر الي الوصول الي البيانات المطلوبة']);
        }
        $data = get_cols_where_p(new main_salary_employee_sanction(), array("*"), array("com_code" => $com_code, "finance_cln_periods_id" => $finance_cln_periods_id), 'id', 'DESC', PC);
        if (!empty($data)) {
            foreach ($data as $info) {
        ...
        return view('admin.Main_salary_employee_sanctions.show', ['data' => $data, 'finance_cln_periods_data' => $finance_cln_periods_data, 'employees' => $employees,'employees_for_search'=>$employees_for_search]);
    }
    public function checkExsistsBefor(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $checkExsistsBeforCounter = get_count_where(new main_salary_employee_sanction(), array("com_code" => $com_code, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, "employees_code" => $request->employees_code));
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $com_code = auth()->user()->com_code;
            $data = main_salary_employee_sanction::select("*")->where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where('finance_cln_periods_id', '=', $the_finance_cln_periods_id)->where('com_code', '=', $com_code)->orderby('id', 'DESC')->paginate(PC);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->emp_name = get_field_value(new employee(), "emp_name", array("com_code" => $com_code, "employees_code" => $info->employees_code));
                    $info->zketo_code = get_field_value(new employee(), "zketo_code", array("com_code" => $com_code, "employees_code" => $info->employees_code));
    ...
    public function delete_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data_row = get_cols_where_row(new main_salary_employee_sanction(), array("id"), array("com_code" => $com_code, "id" => $request->id, 'is_archived' => 0, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, 'main_salary_employee_id' => $request->main_salary_employee_id,));
            if (!empty($finance_cln_periods_data) and !empty($data_row) and !empty($main_salary_employee_data)) {
                destroy(new main_salary_employee_sanction(), array("com_code" => $com_code, "id" => $request->id, 'is_archived' => 0, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, 'main_salary_employee_id' => $request->main_salary_employee_id,));
    ...
    public function load_edit_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            $finance_cln_periods_data = get_cols_where_row(new Finance_cln_periods(), array("id"), array("com_code" => $com_code, "id" => $request->the_finance_cln_periods_id, 'is_open' => 1));
            $main_salary_employee_data  = get_cols_where_row(new Main_salary_employee(), array("id"), array("com_code" => $com_code, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, 'id' => $request->main_salary_employee_id, 'is_archived' => 0));
            $data_row = get_cols_where_row(new main_salary_employee_sanction(), array("*"), array("com_code" => $com_code, "id" => $request->id, 'is_archived' => 0, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, 'main_salary_employee_id' => $request->main_salary_employee_id,));
            ...
            return view('admin.Main_salary_employee_sanctions.load_edit_row', ['finance_cln_periods_data' => $finance_cln_periods_data, 'main_salary_employee_data' => $main_salary_employee_data, 'data_row' => $data_row, 'employees' => $employees]);
        }
    }
    public function do_edit_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($finance_cln_periods_data) and !empty($main_salary_employee_data) and !empty($data_row)) {
                ...
                $dataToUpdate['sanctions_type'] = $request->sanctions_type;
                $dataToUpdate['value'] = $request->value;
                $dataToUpdate['total'] = $request->total;
                $dataToUpdate['notes'] = $request->notes;
                $dataToUpdate['updated_by'] = auth()->user()->id;
                update(new main_salary_employee_sanction(), $dataToUpdate, array("com_code" => $com_code, "id" => $request->id, 'is_archived' => 0, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, 'main_salary_employee_id' => $request->main_salary_employee_id,));
                DB::commit();
    ...
    public function print_search(Request $request)
    {
            ...
            $data = main_salary_employee_sanction::select("*")->where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where('finance_cln_periods_id', '=', $the_finance_cln_periods_id)->where('com_code', '=', $com_code)->orderby('id', 'DESC')->get();
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->emp_name = get_field_value(new employee(), "emp_name", array("com_code" => $com_code, "employees_code" => $info->employees_code));
                    $info->zketo_code = get_field_value(new employee(), "zketo_code", array("com_code" => $com_code, "employees_code" => $info->employees_code));
            ...
            return view('admin.Main_salary_employee_sanctions.print_search', ['data' => $data,'finance_cln_periods_data'=>$finance_cln_periods_data,'systemData'=>$systemData,'other'=>$other]);
        }
    }
